...
Status: **User-approved policy; implementation and validation pending.**

Authority: [MVP design](../design/mvp.md#explicit-retirement-and-replacement), [terminal behavior](../design/pi-terminal-behavior.md#explicit-retirement-and-replacement), and [connection-bound identity ADR](../adr/0003-use-connection-bound-observer-capabilities.md).
...
3. User starts visible Pi themselves, optionally resuming saved history. No automatic Pi launch, history inspection, or session-file identity correlation.
4. Select that Unassigned Pi and the vacant Role, then use the existing exact confirmed/acknowledged Adoption flow. Persist a predecessor link and a new Agent Run identity.
5. Keep uncertain old work blocked for reconciliation. This bounded prototype dispatches zero Assignments.

## Bounded implementation contract

- Extend the existing single runner/store authority, not a parallel registry or UI-owned state machine.
- Persist retirement and Role vacancy atomically with expected Run/revision guards. Preserve immutable original Adoption commitments, takeover records, and binding tombstones. Historical commitments must no longer imply current Role occupancy.
- Persist a Role vacancy generation/predecessor so stale replacements cannot claim a subsequently occupied or re-retired Role. Check it again in the synchronous Adoption transaction.
...
- Retirement revokes orchestration authority only. Do not claim OS-enforced prevention of surviving tool writes. No new Assignment delivery, Return-to-team workflow, automatic process termination, or history deletion in this slice.
...
- failure before/after each durable transition, restart reconstruction, and durable rejection of the retired identity;
...
Human validation is separate and requires explicit installation/live consent: adopt, disconnect, retire, manually resume Pi, adopt the new process into the same Role, verify the old Run remains retired and no automatic work starts. Existing normal Adoption PASS is not replacement evidence. Gateway-crash recovery remains separately untested by the human gate.
