...
Status: **MVP product scope locked; Companion Plugin vertical slice live-proven; observer/Adoption fake-only prototype green; production technical contracts pending**
...
### Responsive dock amendment

The operator approved a whole-dock repair after rejecting the shared one-second
click delay and dense rows: wake the existing Owner on queued clicks, eliminate
redundant IPC negotiation while checking the loaded contract/generation at each
operation, separate liveness from visible updates and preserve row identity/state.
Normal unassigned rows show session code and activity; managed rows retain Role
and control state. Concise exceptions remain visible. The shared Project Adoption
blocker appears once per contextual list rather than beneath every session.
A bounded presentation-only wake client may notify the existing owner endpoint;
it never starts an Owner, Pi, service, terminal, or domain action on its own.
...
**Locked for local presentation.** The persistent Companion supplies a visible Omarchy bar button that opens or reopens the left workbench dock through the running owner. The owner is a single supervised background user service after explicit setup; a persistent owner terminal is not a product requirement. After the operator has explicitly enabled the exact installed user service, the bar button may request systemd to start **only that service** when it is stopped, wait for its verified owner endpoint, and then open the dock. It never spawns the runner process directly, installs a service, starts Pi, or creates a second owner. A missing, foreign, incompatible or failed service reports unavailability rather than pretending the dock opened. The dock has its own Close button. Both actions affect only the Projection Session; they never end Pi, stop the runner, or install/update plugin assets at runtime. If the owner is unavailable, the button reports that fact rather than fabricating an open dock. This is an explicitly requested Boomux-like presentation control, not terminal spawn or PTY attachment. Versioned plugin setup must install/enable the bar entry with exact receipt-backed ownership and rollback; normal hide/reopen must not write `shell.json`.

**Proposed** for additional indicator content. Show:
...
An Observed Pi Session displays `Unassigned · observed` in Omarchestra's named Pi status slot without replacing other extension statuses and retains its ordinary terminal title. The approved shared-identity slice prefixes both that status and the dock row with a negotiated, collision-checked `Pi XXXX-XXXX` display code, retaining it through Adoption and managed state changes within the owner lifetime. It is not durable identity or authority; unavailable/legacy/disconnected codes are explicit. The [slice contract](../reviews/local-workbench-phase-2-verification/session-code-checkpoint.md) records lifetime and compatibility; Companion 0.13.0 is now installed/loaded with the Owner reloaded, while physical matching, latency acceptance and independent review remain pending. Omarchestra may focus a currently correlated ordinary terminal as a presentation convenience, but it does not promise Boomux persistence, reattachment, resize, process control, or exact terminal recovery; Adoption alone does not grant PTY authority or fabricate a managed Runtime Binding. For the local Herdr/Foot ordinary-terminal path (including adopted sessions), the operator-approved **Show terminal pane** action uses checked best-effort navigation through the addressed Pi extension. Pre/post checks constrain pane and original ancestor-window selection; they do not attest current attachment or eliminate dispatch races. Unavailable/unknown outcomes are explicit. This is the bounded exception to exact runtime-bound presentation above, not a change to Boomux or remote lifecycle guarantees.
...
- An explicit **Delete Retired History** action may permanently remove the selected terminal retired Run's durable history and presentation. It retains only a minimal exact-binding security fence, never deletes Pi conversations, session files, processes, tools, or external artifacts, and never exposes the fence as a Run or card. A retired Run with a replacement descendant is blocked until the successor is deleted first; deletion proceeds newest-to-oldest so predecessor linkage remains valid while retained. Vacancy-generation high-water marks survive purge so stale replacement intents never become valid again.
...
3. **Pi bridge, observer, Adoption, and presentation fan-out:** managed-bridge feasibility is closed as supported with constraints. For the bounded observer prototype, [ADR 0003](../adr/0003-use-connection-bound-observer-capabilities.md) selects independent random process/session/extension identity values bound to the exact current local connection and fresh registry/Adoption challenges; the complete prototype envelopes, monotonic ordering/expiry, privacy classes, reconciliation, and atomic observed-to-managed transaction are fixed in the [Observer and Adoption v1 prototype contract](../../prototypes/first-vertical-slice/docs/observer-adoption-v1.md). This selection grants no authority until committed Adoption and is not production attestation or implementation evidence. Production work remains for observer installation, socket trust/permissions, persistence and retention, durable replay/cursors, slash-command and user-bash coverage, broader compatibility, telemetry filtering/coalescing, and one committed role/state value reaching Pi status, title metadata, and Agent Console cards without stale divergence.
...
10. **Workbench service lifecycle:** the panel-to-bar migration, exact rollback/uninstall tests, Companion 0.9.0 installation and physical bar/Close click were completed; see the [resolution](../../spikes/workbench-bar-lifecycle/README.md). After the operator rejected a required foreground Owner terminal, an exact user service was enabled **without starting it**, and an owner-only, unit-identity-checked bar launcher was installed to start the service on demand. Fake verification passes, and the operator confirmed the first real bar click and in-dock Close button worked with the service running in the background; a read-only status check verified the active unit and owner epoch 7. A crash leaving guarded sockets cannot silently erase or take over those resources; service-failure recovery must remain explicit and evidence-backed. Durable ownership receipts must survive Linux device-number changes across reboot by pinning the stable mount source/root/type/mountpoint tuple, Btrfs subvolume ID when applicable, and inode, while the current Owner lifetime checks mount ID and inode/owner/type only; it does not persist or guard on Linux `st_dev`. A failed, start-limit-hit exact enabled service may have only its own systemd failure counter reset by the verified bar launcher before one supervised start; a present but unreachable socket still requires explicit recovery.

11. **Ordinary-session focus:** the [terminal-focus spike](../../spikes/terminal-focus/README.md) found Herdr panes in one Foot window and an unguarded public focus API. The operator subsequently approved checked best-effort **Show terminal pane**, preserving that layout: same-process extension execution, pane/process/window pre/post checks, no mutation on failed preflight, explicit unknown outcome after uncertain mutation, no launch/input/Adoption and no automatic retry. The check/dispatch race is accepted for this presentation-only path; atomic exact-agent focus is not claimed. The [terminal contract](pi-terminal-behavior.md#approved-local-workbench-usability-slices) fixes local metadata/privacy and original-ancestor-window limitations. The [development checkpoint](../reviews/local-workbench-phase-2-verification/checked-pane-navigation.md) now passes disposable bridge/adapter and offscreen tests. Companion 0.13.0 installation and Owner reload are now verified with history preserved. The operator subsequently reports using navigation but rejects latency, flicker and dense rows. The approved [0.14.0 responsive repair](../reviews/local-workbench-phase-2-verification/responsive-dock.md) is disposable-tested and now installed/loaded after a separately authorized shell/Owner restart, with history preserved and the existing manual-takeover Run reconnected; physical acceptance and independent review remain pending.
...
- The operator approved the responsive dock amendment above: event-triggered
  ...
  stable rows and compact session/activity presentation with shared blockers once.
  This changes presentation plumbing, not management authority or installed state.

- Following the Herdr focus spike, the operator explicitly chose to keep the existing layout and approved checked best-effort “Show terminal pane” navigation instead of waiting for guarded runtime APIs. The residual precheck→dispatch→postcheck race is accepted only for presentation. Implement via the exact connected Pi extension, preserve control/membership, and report unavailable/unknown honestly. No terminal creation, input, layout mutation, implicit observer reload or live installation is authorized.

- The operator approved three [local-workbench session-usability slices](../plans/workbench-session-usability.md): truthful activity/health and explicit Adoption-unavailable reasons; a collision-checked visual identity shared by dock row and Pi status; and exact terminal-focus navigation with stale/ambiguous correlation refusal. Display codes grant no authority, ordinary terminal titles remain unchanged, and focus does not adopt, dispatch, launch a replacement or grant PTY persistence. Implement and gate each slice separately; live installation, Adoption and desktop checks remain separately authorized.

- 2026-09-25: The operator rejected the in-dock Adoption confirmation box shown after `Adopt as implementer` and explicitly selected a **second press of that same button** as the confirmation gesture. This supersedes the 2026-09-24 inline-review presentation decision below, not the exact-target or same-process Adoption authority contract. Consequential enabled actions arm their original button on the first press, visibly relabel it `Confirm: …`, and submit only on a second press of the same action with the same session, Goal, Role, generation and relevant runner state within 30 seconds. Escape, navigation, disconnection, expiry or relevant change disarms it; no separate box, overlay, notification or focus jump remains. An obsolete second press can only re-arm a current action, never authorize the old one. Runner validation, acknowledgement and no-dispatch-before-commit remain unchanged. This is approved presentation direction; offscreen tests are not physical-click acceptance.

- 2026-09-25: During the same-button Companion 0.11.0 update, a failed live installer attempt exposed a separate fail-closed recovery bug: atomic file replacement changes inodes, but its fake model had preserved them and a failure rollback wrote the old bytes under new inodes without updating the old ownership receipt. Future updates stage all original owned assets for exact-inode restoration on failure; the fake filesystem now models real atomic inode changes. For this one affected v0.10.0 installation, a separate exact-evidence, backed-up receipt-only rebind was authorized in a visible TTY and validated before a second, separately authorized 0.11.0 update. Current receipt/installed assets match; unrelated `shell.json` bytes and the persistent Goal history were retained. This is not permission to rebind foreign resources, silently waive receipt checks or install a Companion during normal Goal/Projection operation. Evidence and live limits: [same-button correction](../reviews/local-workbench-phase-2-verification/same-button-confirmation.md).

- 2026-09-25: A real login restart exposed a defective ownership receipt: the owner pinned `/run/user` and its private runtime-directory inodes as if they were durable, causing a safe but unusable `identity_drift`/start-limit failure. Keep exact permanent-state and one-owner locking unchanged, pin runtime identities within each owner lifetime, and explicitly migrate the existing version-1 receipt under an exclusive lock with preserved evidence. This restores the already-approved background owner's login behavior; it does not claim reboot recovery of Pi processes or model sessions.
- 2026-09-26: A subsequent login restart reproduced `identity_drift` because the Btrfs-backed `/home` tree retained every recorded inode but Linux reported a different `st_dev` (`58`→`53`). Durable receipts now use the stable mount source/root/type/mountpoint tuple, Btrfs subvolume ID when applicable, and inode/owner/type; the current Owner lifetime guards mount ID and inode/owner/type only, never `st_dev`. Legacy device fields are inspected only to validate an explicit migration and are omitted from v3 receipts and runtime guards. A plan-bound v1/v2→v3 receipt migration preserves the original receipt, and the verified bar launcher may reset failure state only for the exact enabled Owner service before a supervised start. This does not bypass receipt validation or clear stale sockets.

- 2026-09-24: After seeing the short-lived Adoption confirmation overlay in the actual dock, the operator rejected such pop-up dialogs throughout the workbench. Consequential actions keep their exact-target review and explicit confirmation, but render it inline in the fixed dock, not in a modal or desktop notification. Ordinary heartbeat/snapshot refreshes must not close the review. Expiry or relevant authority change disables confirmation and leaves a clear, dismissible inline explanation; it never silently authorizes stale work. A follow-up screenshot revealed a separate `notify-send` error pop-up from the bar launcher's failure path; that desktop notification was removed as well. A plugin update requires both shell reload and restart of the long-lived owner service to load matching runtime version constants; failure to do both must report unavailable, not fake success.

- 2026-09-24: The operator explicitly authorized enabling the local owner as a systemd user service **without starting it during setup**, and requested that the bar button start that exact enabled service on demand. The action may start only the verified installed service through systemd and must wait for real owner readiness before opening the dock; it is not authorization to create a Pi, bypass ownership checks, auto-install service state, or treat `enable` as `--now`.

- 2026-09-24: After the physical bar/Close walkthrough, the operator rejected a foreground terminal as a required owner lifecycle. The foreground Phase-2 owner was a temporary engineering harness, not an acceptable installed-product dependency. Restore the already-selected systemd user-service default for the local owner through a separate explicit setup/enable step with one-owner locking, owner-only roots, honest unavailable reporting and no implicit bar-click process creation; never spawn a second Pi. Full reboot recovery of agent/model sessions remains deferred.

- 2026-09-24: The operator explicitly requested a Boomux-like Omarchy bar button to reopen the persistent workbench dock and a Close button in the dock. These are presentation-only controls: the owner stays alive, Pi processes and managed bindings remain untouched, and installation of the bar entry requires exact authorized Companion setup. A user-local app-launcher toggle is only an interim workaround, not acceptance of the bar button.

- 2026-09-24: The operator rejected fixed Omarchy package-version gating for the active Local Workbench Companion. [ADR 0005](../adr/0005-negotiate-companion-features-not-host-package-versions.md) replaces it with API and loaded-component negotiation, preserving exact installation ownership/authorization and version-change freshness checks but not a fixed version allowlist. Do not alter historical release bytes or interpret absence of a version pin as universal compatibility.
