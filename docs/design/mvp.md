...
An Observed Pi Session displays `Unassigned · observed` in Omarchestra's named Pi status slot without replacing other extension statuses and retains its ordinary terminal title. The approved shared-identity slice prefixes both that status and the dock row with a negotiated, collision-checked `Pi XXXX-XXXX` display code, retaining it through Adoption and managed state changes within the owner lifetime. It is not durable identity or authority; unavailable/legacy/disconnected codes are explicit. The [slice contract](../reviews/local-workbench-phase-2-verification/session-code-checkpoint.md) records lifetime and compatibility; implementation is development-only, not live acceptance. Omarchestra may focus a currently correlated ordinary terminal as a presentation convenience, but it does not promise Boomux persistence, reattachment, resize, process control, or exact terminal recovery; Adoption alone does not grant PTY authority or fabricate a managed Runtime Binding.
