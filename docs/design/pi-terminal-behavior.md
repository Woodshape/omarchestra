...
The user may explicitly retire a disconnected or exited Agent Run, then adopt a new observed Pi into the vacated Role. Retirement permanently fences the old Run's orchestration authority and preserves its history while retained; it does not kill Pi or delete its session file. A separate explicit Delete Retired History action may remove a terminal retired Run from Omarchestra's durable records, but never deletes Pi history, session files, processes, tools, or external artifacts. A predecessor with a replacement descendant remains until successors are purged first. A reconnecting retired extension cannot reclaim the Role.
