...
6. **Workbench bar lifecycle — read-only assessment, migration contract open.** The installed Omarchy API cannot promote the receipt-owned panel-only entry to the same-ID bar widget via enable/put alone. No live mutation occurred. See [`workbench-bar-lifecycle/README.md`](workbench-bar-lifecycle/README.md).
