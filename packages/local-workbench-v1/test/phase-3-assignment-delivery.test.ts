...
function setup(t: TestContext, options: { unsupportedApi?: boolean; sendThenThrow?: boolean } = {}): Fixture {
  ...
  const makePi = () => {
    const pi: Record<string, unknown> = { on(name: string, fn: (event: unknown, ctx: unknown) => void) { hooks.set(name, fn) } }
  ...
  return { root, runner, registry, authority, projectId, goalId, streams, hooks, host, calls, deadlineTicks, coordinator, adopt, seed, taskText: TASK_TEXT, now: () => now, setNow: value => { now = value } }
}
